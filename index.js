...
parser.add_argument('--reportrepo', { default: '', help: 'repo to upload report S3 or GCP, e.g. S3' });
...
process.env.REPORTREPO = args.reportrepo;
process.env.TAGS = args.tags;
process.env.REPORTREPO = args.reportrepo;

// Write the arguments to a JSON file
...
const command = `run cucumber -- --tags "${args.tags}" ${rerunCommand} ${args.report ? `${commandChainOperator} npm run report ${commandChainOperator} npm run scenario-reports` : ''}`;
